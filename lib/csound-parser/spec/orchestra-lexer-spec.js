...
describe('Csound orchestra lexer', () => {
  ...
  function tokens(string) {
    ...
    Object.assign(lexer.symbolTable.identifiers, builtInOpcodeSymbolTable.identifiers);
    while (!lexer.done) {
  ...
  it('warns about duplicate label', () => {
    ...
    expect(lexer.messages[0]).toEqual({
      ...
      trace: [{
        type: 'Trace',
        text: 'Label ‘label’ is defined here',
        range: [[0, 0], [0, 5]]
      }]
    });
  });
  ...
  it('lexes opcode definition', () => {
    ...
    expect(tokens(`
      opcode ${opcodeName} , 0 , 0
      endop
    `)).toEqual(['NEWLINE', 'OPCODE', 'IDENTIFIER', ',', 'OPCODE_OUTPUT_TYPES', ',', 'OPCODE_INPUT_TYPES', 'NEWLINE', 'ENDOP', 'NEWLINE']);
    expect(lexer.messages.length).toBe(0);

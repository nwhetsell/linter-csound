...
const parser = require('vm').runInThisContext(code, {filename: filename})(require);
...
describe('Csound orchestra lexer', () => {
  ...
  function tokens(string) {
    ...
    Object.assign(lexer.globalSymbolTable.identifiers, SymbolTable.builtInOpcodeSymbolTable.identifiers);
    while (!lexer.done) {
  ...
  it('lexes unexpected characters', () => {
    expect(tokens(dedent`
      instr 1
        ...
        \`@.\\$•🎹
      endin
    `)).toEqual(['INSTR', 'DECIMAL_INTEGER', 'NEWLINE', 'VOID_OPCODE', 'STRING', 'NEWLINE', 'NEWLINE', 'ENDIN']);

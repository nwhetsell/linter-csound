...
const lexer = (function() {
      ...
      switch (yyrulenumber) {
        ...
        case 23:
          ...
          this.messages.push({
            ...
            excerpt: `${this.quote(
              yy_.yytext
            )} must be replaced with ${this.quote(solution.replaceWith)}`,
            solutions: [solution]
          });
          break;
        case 24:
        ...
        case 33:
          ...
          if (parameterNames.indexOf(yy_.yytext) >= 0) {
            throw new CsoundPreprocessorError({
              ...
              excerpt: `Duplicate macro parameter name ${this.quote(
                yy_.yytext
              )}`
            });
          }
        ...
        case 44:
          ...
          if (yy_.yytext.length > 0) throw new CsoundPreprocessorError(message);
          this.messages.push(message);
          break;
        case 45:
        ...
        case 52:
          ...
          this.popState();
          this.begin(
            this.macrosByName[yy_.yytext] ? "ifdef_true" : "ifdef_false"
          );
          break;
        case 53:
        ...
        case 64:
          ...
          {
            const includeRange = this.rangeFromPosition(
              yy_.yylloc.first_line,
              yy_.yylloc.first_column
            );
            ...
            if (delimiter !== '"') {
              this.messages.push({
                ...
                excerpt: `${this.quote(delimiter)} instead of ${this.quote(
                  '"'
                )} used to enclose file path`
              });
            }
            let includeFilePath = "";
            for (
            ...
            if (includeFilePath.length > 0) {
              ...
              if (path.isAbsolute(includeFilePath)) {
                ...
                if (this.filePath) paths.push(path.dirname(this.filePath));
                ...
                paths = paths.map(directory =>
                  path.join(directory, includeFilePath)
                );
              }
              for (const absolutePath of paths) {
                ...
                if (stats.isFile()) {
                  ...
                  const preprocessor = this.makePreprocessor(
        ...
        case 71:
          ...
          this.popState();
          this.addText(
            Math.pow(2, Math.ceil(Math.log2(Number(yy_.yytext) + 1))).toString()
          );
          break;
        case 72:
          ...
          this.addText(
            yy_.yytext === "0"
              ? "2"
              : (Math.pow(2, Math.ceil(Math.log2(Number(yy_.yytext)))) +
                  1).toString()
          );
          break;
        case 73:
        ...
        case 87:
          ...
          for (
            let repitition = 1;
            repitition <= this.macro.repeatCount;
            repitition++
          ) {
            this.macroUse = new MacroUseElement(
              this.macro,
            ...
            if (repititionMacro) {
              ...
              this.macroUse.macrosByName[
                repititionMacro.name
              ] = repititionMacro;
            }
            this.expandMacro(YY_START);
          }
  ...
  lexer.expandMacro = function(YY_START) {
    ...
    if (
      ...
      Object.keys(macrosByName).length < macro.parameterNames.length
    ) {
      throw new CsoundPreprocessorError({
        severity: "error",
        location: {
          file: this.filePath,
          position: lexer.rangeFromPosition(
  ...
  class LineMap {
    ...
    sourceLocation(column) {
      for (; column >= 0; column--) {
        ...
        if (map) return [map.sourceLine, map.sourceColumn];
      }
      return null;
    }
  }
  ...
  class SourceMap {
    ...
    sourceLocation(location) {
      for (let line = location[0]; line >= 0; line--) {
        ...
        if (lineMap) return lineMap.sourceLocation(location[1]);
      }
      return null;
    }
